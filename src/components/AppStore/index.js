...
class AppStore extends Component {
  state = {searchInput: '', cate: tabsList[0].tabId}
  ...
  onChangeCategory = tabId => {
    this.setState({cate: tabId})
  }
  filtered = () => {
    const {cate} = this.state
    const filteredList = appsList.filter(
      eachProject => eachProject.category === cate,
    )
    return filteredList
  }

  render() {
    const {searchInput, cate} = this.state

    const filteredList = this.filtered()
    const searchedResult = filteredList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="app">
        <h1 className="hea">App Store</h1>
        ...
        <ul className="tabs-con">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId === cate}
            />
          ))}
        </ul>
        <ul className="apps-items-con">
          {searchedResult.map(eachApp => (
            <AppItem appsDetails={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
